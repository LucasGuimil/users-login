...
export function validateId(req, res, next) {
    ...
    if (req.params.cid) {
        ...
        if (req.params.cid != req.user.cart) { return res.status(403).json({ message: "That is not your cart ID." }) }
        if (!req.user.cart) { return res.status(400).json({ message: "Cart is not created." }) }
    }
    if (req.params.pid) {
